...
For example, suppose there is one computer with 10 CPU cores and 10 GB of memory, and 3 pods running on it. You could run around and allocate the compute between the pods. But the purpose of Kubernetes is to have it do it for you, it dynamically scales the resources to each pods so they can all run safely. Another example is imagine the pods need to talk to each other via an IP address, Kubernetes can do this for you to make it much easier to do it for you.
#### Availability
